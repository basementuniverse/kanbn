...
module.exports = {
  ...
  md2json(data) {
    ...
    try {
      ...
      metadata = 'Metadata' in parsed[name]
        ? yaml.parse(parsed[name]['Metadata'].raw.trim().replace(/```(yaml|yml)?/g, ''))
        : {};
      subTasks = 'Sub-tasks' in parsed[name]
        ? marked.lexer(parsed[name]['Sub-tasks'].raw)[0].items.map(item => ({
          text: item.text.trim(),
          completed: item.checked
        }))
        : [];
      relations = 'Relations' in parsed[name]
        ? marked.lexer(parsed[name]['Relations'].raw)[0].items.map(item => {
