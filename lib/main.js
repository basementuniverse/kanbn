...
module.exports = (() => {
  ...
  function taskInIndex(index, taskId) {
    for (let columnName of index.columns) {
   ...
   * @param {object} index The index object
   * @param {*} taskId The task id to rename
  ...
  async function saveIndex(indexData) {
    await fs.promises.writeFile(INDEX, parseIndex.jsonToIndex(indexData));
  }
  ...
  async function saveTask(path, taskData) {
    await fs.promises.writeFile(path, parseTask.jsonToTask(taskData));
  }
