...
// TODO caching for search and status
//  if cache settings are available (enableCache and cacheFile in index options), use the cache for search and status
...
module.exports = (() => {
  ...
  function getTrackedTasks(index) {
    return new Set(Object.keys(index.columns).map(columnName => index.columns[columnName]).flat());
  }
   ...
   */
  async function loadAllTrackedTasks(index) {
    const result = {};
   ...
   * @param {string} filter A substring or regex
  ...
  function stringFilter(filter, input) {
    ...
    if (filter.startsWith('/') && filter.endsWith('/')) {
      return (new RegExp(filter.substring(1, filter.length - 1))).test(input);
    }
  ...
  function dateFilter(dates, input) {
    ...
    dates = dates.map(date => chrono.parseDate(date));
    if (dates.length === 1) {
  ...
  return {
    ...
    async createTask(taskData, columnName) {
      ...
      const taskId = getTaskId(taskData.name);
    ...
    async updateTask(taskId, taskData, columnName = null) {
      ...
      if (originalTaskData.name !== taskData.name) {
        taskId = this.renameTask(taskId, taskData.name);
      }
      ...
      if (columnName) {
        this.moveTask(taskId, columnName);
      }
    ...
    async renameTask(taskId, newTaskName) {
      ...
      const newTaskId = getTaskId(newTaskName);
    ...
    async search(filters = {}, quiet = false) {
      ...
      const index = await loadIndex();
      const tasks = loadAllTrackedTasks(index).filter(task => {
        ...
        // Name
        if ('name' in filters && !stringFilter(filter.name, task.name)) {
          result = false;
        }

        // Description
        if ('description' in filters && !stringFilter(filter.description, task.description)) {
          result = false;
        }
        ...
        if (
          'created' in filters && (
            ...
            !dateFilter(filter.created, task.metadata.created)
          )
        ) {
          result = false;
        }
        ...
        if (
          'updated' in filters && (
            ...
            !dateFilter(filter.updated, task.metadata.updated)
          )
        ) {
          result = false;
        }
        ...
        if (
          'completed' in filters && (
            ...
            !dateFilter(filter.completed, task.metadata.completed)
          )
        ) {
          result = false;
        }
        ...
        if (
          'due' in filters && (
            ...
            !dateFilter(filter.due, task.metadata.due)
          )
        ) {
          result = false;
        }
      ...
      return tasks.map(task => {
        return quiet ? getTaskId(task.name) : task;
      });
    },
    ...
    async status(quiet = false) {
      ...
      const result = {
        totalTasks: index.columns.reduce((a, v) => a + v.length, 0),
      ...
      if (!quiet) {
        const tasks = loadAllTrackedTasks(index);
        // TODO kanbn.status non-quiet
      }
      ...
      if sprint settings are defined in options:
      total workload of tasks created this sprint
      total workload of completed this sprint

      **tasks are worth 2 points by default
        some tags can change this value:
        - nothing 0
        - tiny 1
        - small 2
        - medium 3
        - large 5
        - huge 8
        to calculate workload, add up task values
      */
    },
