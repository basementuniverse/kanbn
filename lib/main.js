...
module.exports = (() => {
   ...
   * @param {Date[]} dates A list of dates to check against
   ...
   * @param {number} input The count to match against
   * @return {boolean} True if the input matches the string filter
   */
  ...
  return {
     ...
     * @param {boolean} json Show output in JSON format, otherwise use YAML
     * @param {boolean} untracked Show a list of untracked tasks
     * @param {?number} sprint The sprint number to show stats for, or null for current sprint
     * @param {?Date[]} dates The date(s) to show stats for, or null for no date filter
     * @return {object} Project status information as an object
     */
    async status(quiet = false, json = false, untracked = false, sprint = null, dates = null) {
      ...
      if (untracked) {
        result.untrackedTasks = [...this.findUntrackedTasks()].map(taskId => `${taskId}.md`);
      ...
      if (!quiet) {
        ...
        // Get task workloads from index or defaults
        ...
        const columnWorkloads = tasks.reduce(
          (a, task) => {
            ...
            a[findTaskColumn(index, task.id)] += task.workload;
            return a;
          },
        ...
        if ('sprints' in index.options && index.options.sprints.length) {
          ...
          // If sprint number arg is set, use the specified sprint otherwise use current sprint
            // get start/end dates
        // Elseif any dates args
          // get start/end dates or single date

        // If dates are set
          // calculate workload of tasks created, started and completed in this duration
        // else
          // calculate total workload of tasks created, started and completed of all time
    ...
    async validate() {
      // TODO kanbn.validate
      // load index and all task files in try/catch, collate errors
    },
    ...
    async sprint(name, description, start) {
      ...
      const sprint = {
        number: index.options.sprints.length + 1,
        start: start
      };
